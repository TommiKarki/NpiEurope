...
for(i in 1:length(countryVec)){
  ...
  temp=summarise_estimation(sprintf("%s/MCMC_%s.csv",folder,country), country_data,contact_data,age_data);
...
for (i in 1:length(countryVec)) {
  ...
  res=summarise_estimation(sprintf("%s/MCMC_%s.csv",folder,country), country_data,contact_data,age_data);
...
combStrat<-c()
for(i in 1:length(matResults[,1])){
